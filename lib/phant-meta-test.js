...
app.update = function(id, data, callback) {
  ...
  var diff = _.difference(_.keys(data), ['title', 'flagged', 'description', 'fields', 'tags', 'hidden', 'custom', 'last_push']),
      record = _.findWhere(this.streams, { id: id });

  if(diff.length !== 0) {
    return callback('update failed: invalid data sent');
  }
